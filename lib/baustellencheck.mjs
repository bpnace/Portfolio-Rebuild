...
export const goalOptions = [
  ...
  {
    value: "complete-relaunch",
    label: "Kompletter Relaunch",
  },
  {
    value: "web-app-extension",
    label: "Web App oder technische Erweiterung",
  },
];
...
export const publicInspectionFields = inspectionFields.filter((field) =>
  ["Fundament", "Grundriss", "Fassade", "Eingang"].includes(field.title),
);
...
export function validateBaustellencheckPayload(payload) {
  ...
  if (!isTermsAccepted(payload?.terms)) {
    errors.terms =
      "Bitte bestätige die Verarbeitung deiner Angaben für den Baustellencheck.";
  }
...
export function buildBaustellencheckSummary(data) {
  const lines = [
    ...
    `Bauzustand: Offen`,
    `Empfohlener nächster Schritt: Offen`,
...
export function buildForwardedBaustellencheckPayload(data, payload = {}, request) {
  ...
  const source =
    cleanText(payload.source, MAX_SHORT_TEXT_LENGTH) ||
    cleanText(payload.origin, MAX_SHORT_TEXT_LENGTH) ||
    "STACKWERKHAUS Baustellencheck";
  ...
  return {
    ...
    last_contact: submittedAt,
    next_action: "Website prüfen und Bauzustand einordnen",
    name: data.name,
    email: data.email,
    ...
    privacy_accepted_at:
      cleanText(payload.privacy_accepted_at, MAX_SHORT_TEXT_LENGTH) ||
      submittedAt,
    newsletter_opt_in_at: data.newsletterOptIn
      ? cleanText(payload.newsletter_opt_in_at, MAX_SHORT_TEXT_LENGTH) ||
        submittedAt
      : "",
    terms: true,
    website: "",
    ...
    confirmation_email: {
      subject: "Dein Stackwerkhaus Baustellencheck ist eingegangen",
      intro:
        "deine Website ist bei uns zur Bauaufnahme eingegangen.",
      response_time: "In der Regel melden wir uns innerhalb von 24 bis 48 Stunden.",
    },
  };
}
